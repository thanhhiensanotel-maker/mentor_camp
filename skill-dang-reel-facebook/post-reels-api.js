...
const CFG = {
  APP_ID:        process.env.LARK_APP_ID     || 'cli_xxxxxxxxxxxxxxxx',   // Lark app_id (Variable/payload)
  APP_SECRET:    process.env.LARK_APP_SECRET || '',                       // BẮT BUỘC qua env (Secret)
  APP_TOKEN:     process.env.LARK_APP_TOKEN  || 'bascnXXXXXXXXXXXXXXXX',  // base token (Variable/payload)
  TABLE_ID:      process.env.LARK_TABLE_ID   || 'tblXXXXXXXXXXXXXX',      // bảng "Đăng Reel" (Variable/payload)
  FB_PAGE_ID:    process.env.FB_PAGE_ID      || '',                       // page id (Variable/payload)
  FB_PAGE_TOKEN: process.env.FB_PAGE_TOKEN   || '',                       // BẮT BUỘC qua env (Secret)
...
if (!DRY && (!CFG.APP_SECRET || !CFG.FB_PAGE_TOKEN || !CFG.FB_PAGE_ID)) {
  console.error('!! Thiếu biến bắt buộc: cần LARK_APP_SECRET (Secret), FB_PAGE_TOKEN (Secret) và FB_PAGE_ID.');
  console.error('   Đặt Secrets/Variables trên GitHub hoặc truyền FB_PAGE_ID/LARK_* qua client_payload.');
  process.exit(1);
}
